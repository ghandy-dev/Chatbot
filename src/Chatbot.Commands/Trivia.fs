...
module Trivia =
    ...
    open Chatbot.Core.Services.Trivia

    let private keys = [ "count" ; "exclude" ; "include" ; "hints" ]

    let private replacePattern = "[A-Za-z0-9\(\)\[\]]"

    let trivia (triviaService: ITriviaService) context =
        asyncResult {
            ...
            | Channel (channel, _) ->
                ...
                | "stop" :: _ -> return [ CommandResponse.stopTrivia channel ]
                | args ->
                    ...
                    let excludeCategories = kvp.KeyValues.TryFind "exclude" |> Option.map (_.Split(",", System.StringSplitOptions.TrimEntries) >> List.ofArray) |> Option.defaultValue []
                    let includeCategories = kvp.KeyValues.TryFind "include" |> Option.map (_.Split(",", System.StringSplitOptions.TrimEntries) >> List.ofArray) |> Option.defaultValue []
                    let hints = kvp.KeyValues.TryFind "hints" |> Option.bind tryParseBoolean |? true

                    let! questions = triviaService.GetQuestions count excludeCategories includeCategories |> AsyncResult.mapError (CommandHttpError.fromHttpStatusCode "Trivia")
                    ...
                    | questions ->
                        ...
                        return [
                            CommandResponse.startTrivia triviaConfig
                            Message "Trivia starting..."
                        ]
        }