...
module Messages =
    ...
    module Command =
        ...
        let [<Literal>] Notice = "NOTICE"
        let [<Literal>] Part = "PART"
        let [<Literal>] Ping = "PING"
        let [<Literal>] PrivMsg = "PRIVMSG"
        let [<Literal>] Reconnect =  "RECONNECT"
        let [<Literal>] RoomList = "353"
        let [<Literal>] RoomState = "ROOMSTATE"
        let [<Literal>] UserNotice = "USERNOTICE"
        let [<Literal>] UserState = "USERSTATE"
        let [<Literal>] Whisper = "WHISPER"
        let [<Literal>] Unsupported = "421"
    ...
    module Mapping =
        ...
        let (|ClearChatCommand|_|) (message: MessageData) : ClearChatMessage option =
            match message.Command with
            | Command.ClearChat  ->
                Some {
        ...
        let (|ClearMsgCommand|_|) (message: MessageData) : ClearMsgMessage option =
            match message.Command with
            | Command.ClearMsg ->
        ...
        let (|HostTargetCommand|_|) (message: MessageData) : option<HostTargetMessage> =
            match message.Command with
            | Command.HostTarget ->
        ...
        let (|JoinCommand|_|) (message: MessageData) : JoinMessage option =
            match message.Command with
            | Command.Join ->
                Some {
        ...
        let (|PartCommand|_|) (message: MessageData) : PartMessage option =
            match message.Command with
            | Command.Part ->
                Some {
                    Channel = message.Parameters
        ...
        let (|PingCommand|_|) (message: MessageData) : PingMessage option =
            match message.Command with
            | Command.Ping -> Some({ Message = message.Parameters[1..] }: PingMessage)
            | _ -> None

        let (|PrivateMessageCommand|_|) (message: MessageData) : PrivateMessage option =
            match message.Command with
            | Command.PrivMsg ->
                let parts = message.Parameters.Split(" ", 2)
                let msg = parts.[1].[1..]
        ...
        let (|ReconnectCommand|_|) (message: MessageData) : IrcMessage option =
            match message.Command with
            | Command.Reconnect -> Some ReconnectMessage
            | _ -> None
        ...
        let (|UserNoticeCommand|_|) (message: MessageData) : UserNoticeMessage option =
            match message.Command with
            | Command.UserNotice ->
                let (channel, msg) =
        ...
        let (|UserStateCommand|_|) (message: MessageData) : UserStateMessage option =
            match message.Command with
            | Command.UserState ->
                Some {
                    Channel = message.Parameters.[1..]
        ...
        let (|WhisperCommand|_|) (message: MessageData) : WhisperMessage option =
            match message.Command with
            | Command.Whisper ->
                let parts = message.Parameters.Split(" ", 2)
                let msg = parts.[1].[1..]
